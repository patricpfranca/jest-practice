...
import filter from "../src/filter/filter-recursive";
...
it("filter([1, 2, 3, 5], (item, index) => index === index + 1) should return [1, 2, 3]", () => {
  expect(filter([1, 2, 3], (item, index) => index + 1)).to.be.deep.equal([
    1,
    2,
    3
  ]);
